...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{\emph{阈值}}
\end{subfigure}
\caption{在三种判定下直径 $D=10$ 的同一圆周。每幅图显示对应版本的不等式 $d_x^2+d_y^2\leq 1$ 所标记的单元集合,使用 Minecraft 的 \emph{cobblestone} 材质渲染。注意行间过渡、角点单元和基本方位的差异——三种在不同判定下都数学正确的答案。}
\label{fig:comp-d10}
\end{figure}
...
\begin{minec}
\textbf{Minecraft 映射。} 对于 $D=20$ 的玻璃圆顶（中空），粗模式产生 $608$ 块的\emph{密封}外壳，细模式约 $510$ 块 —— 多出的约 $100$ 块是对角桥补充了细模式留下的对角空体素，防止环境光从这些空气格漏入。
\end{minec}
...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{\emph{细 (\emph{1 体素})}}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.30\linewidth}\centering
  \includegraphics[width=\linewidth]{math_2d_d20_thick.png}
  \caption{\emph{粗}}
\end{subfigure}
\caption{同一 $D=20$ 欧几里得圆周的三种渲染模式。\emph{细}模式对应离散拓扑边界 ($\partial F$);\emph{粗}模式增加对角块以封闭 $45^\circ$ 的孔洞,这对密封构造(水下玻璃/冰穹顶)至关重要。}
\label{fig:modes}
\end{figure}
...
\begin{subfigure}[t]{0.46\linewidth}\centering
  ...
  \caption{球体 $D=10$, $r_x=r_y=r_z=5$.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.46\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_ellipsoid.png}
  \caption{椭球体 $W=20,\,H=10,\,D=12$.}
\end{subfigure}
\caption{$\mathbb{R}^3$ 中的体素化。每个立方体是一个体素;以其中心坐标 $(i,j,k)$ 代入隐式方程 $a_x^2+a_y^2+a_z^2\leq 1$ 进行检验。表面上可见的阶梯是离散化的数学必然结果——正是赋予 Minecraft 视觉标识的特性。}
\label{fig:3d-shapes}
\end{figure}
...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{对角 50\%}
\end{subfigure}
\caption{对同一 $D=16$ 球体的三种切法,均保留 50\% 体积。$Y$ 切产生经典穹顶 $V_{\mathrm{穹顶}}=\tfrac{2}{3}\pi r^3$;对角切暴露出该切割平面特有的斜边 $x+y=k$。}
\label{fig:cuts}
\end{figure}
...
\]
结果随后与切割保留的集合相交。背后理论是\acc{数字拓扑}：粗算子保证外壳的 \emph{26-连通性}，对 Minecraft 建造有用，因为对角相邻体素不通过面相接 —— 光、水和生物穿过这些对角的空体素（即空气方块）就像墙不在那里。

\begin{minec}
\textbf{Minecraft 映射。} 对于生存模式中海底神殿上的水下玻璃圆顶，粗模式是强制的：对角桥填补了细模式留下的对角空体素（空气方块），从而阻止水从这些空气格渗入。代价：$D=20$ 粗模式圆顶约需 $380$ 块玻璃，对比细模式约 $320$ 块 —— 多出 $19\%$ 材料换取完全密封。
\end{minec}
...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{\emph{Classic} (强对比)}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.30\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_shading_blocks.png}
  \caption{\emph{Blocks} (默认)}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.30\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_shading_smooth.png}
  \caption{\emph{Smooth} (面相近)}
\end{subfigure}
\caption{对同样三个可见面应用三种光照倍数。\emph{Classic} 给出标志性的 Minecraft 原版外观;\emph{Smooth} 降低对比度以呈现平滑表面;\emph{Blocks} 为中间默认值,其几何内嵌即使在材质相同时也能显露体素边界。}
\label{fig:shading}
\end{figure}
...
\begin{subfigure}[t]{0.40\linewidth}\centering
  ...
  \caption{叠加 \emph{ON}(默认)}
\end{subfigure}
\caption{同一体素集合在外露棱上有/无黑色轮廓的对比。开启 \emph{highlight overlay} 时,外壳上缺失的方块会产生肉眼一瞥即可察觉的图案不连续——生存建造中的快速验证工具。}
\label{fig:overlay}
\end{figure}
...
\caption{$D=10$ 球体,正八分卦象 $(+x,+y,+z)$ 以绿色高亮。在子群 $\mathbb{Z}_2^3 \leq O_h$(阶 8,由三个坐标反射生成)的作用下,该八分卦象决定整个球——其余七个区域通过对手工构建的八分卦象施加一系列带 \emph{mode:masked} 的 \cmd{/clone} 调用得到。}
\label{fig:octants}
\end{figure}
...
\end{subfigure}
\caption{同一 $D=8$ 球体使用三种不同方块渲染。体素化轮廓(数学)在三者中完全相同:算法选定体素;材质是事后施加的纯粹美学层,既不改变体积也不改变拓扑。}
\label{fig:textures}
\end{figure}
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Bresenham, J. E.} \emph{Algorithm for computer control of a digital plotter}.
        ...
        可获取于 \biblink{https://doi.org/10.1147/sj.41.0025}{doi:10.1147/sj.41.0025}.
        --- 本文档第 §5 节所讨论的整数中点算法的原始论文。
  ...
  \item \textbf{Pitteway, M.~L.~V.} \emph{Algorithm for drawing ellipses or hyperbolae
        ...
        可获取于 \biblink{https://doi.org/10.1093/comjnl/10.3.282}{doi:10.1093/comjnl/10.3.282}.
        --- 将 Bresenham 算法推广至任意圆锥曲线,是第 §5 节中椭圆扩展的基础。

  \item \textbf{Kappel, A.} \emph{An ellipse-drawing algorithm for raster displays}.
        收录于 \emph{Fundamental Algorithms for Computer Graphics} (R.~A.~Earnshaw, ed.),
        NATO ASI Series F-17, Springer, 1985, p.~257--280.
        --- 椭圆实现中所采用的两区域公式。

  \item \textbf{Klette, R.; Rosenfeld, A.} \emph{Digital Geometry: Geometric Methods
        for Digital Picture Analysis}. Morgan Kaufmann, 2004. 656 页.
        --- 数字拓扑、边界算子 ($\partial F$)、4-/6-/26-连通性的标准参考。用于 §7、§12 和 §18。
  ...
  \item \textbf{Foley, J.~D.; van Dam, A.; Feiner, S.~K.; Hughes, J.~F.}
        ...
        --- 光栅化、光照模型、透视投影的经典论述。与 §13--15 相关。
\end{itemize}

\subsection*{所引用的 Minecraft 规范与工具}
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  ...
  \item \textbf{Mojang AB.} \emph{Minecraft Wiki --- Commands/fill, /clone}.
        ...
        --- §17 与 §20 中引用的原版命令 \cmd{/fill} 与 \cmd{/clone}。

  \item \textbf{Masady} (\emph{Litematica} mod).
        \biblink{https://github.com/maruohon/litematica}{github.com/maruohon/litematica}.
        --- 导入示意图并以半透明幽灵显示待施工建筑的 Minecraft Java 版模组。
\end{itemize}

\subsection*{几何、线性代数与群论}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{王萼芳, 石生明 (编)} \emph{高等代数}. 高等教育出版社, 北京, 2003.
  ...
  \item \textbf{Armstrong, M.~A.} \emph{Groups and Symmetry}. Undergraduate Texts in
        ...
        --- 八面体群 $O_h$ 及其子群的入门论述,用于 §20。

  \item \textbf{Coxeter, H.~S.~M.} \emph{Regular Polytopes}. 第3版, Dover, 1973.
        --- 正多面体对称群的经典参考,包含 §20 中提及的 48 阶 $O_h$ 的详细处理。

  \item \textbf{Hilbert, D.; Cohn-Vossen, S.} \emph{Geometry and the Imagination}.
        AMS Chelsea, 1990 [orig.~1932].
        --- 二次曲面与几何可视化的经典论述,§9 的直观基础。
\end{itemize}

\subsection*{算法、数据结构、复杂度}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Cormen, T.~H.; Leiserson, C.~E.; Rivest, R.~L.; Stein, C.}
        \emph{Introduction to Algorithms}. 第4版, MIT Press, 2022.
        --- §4--6 三种算法的复杂度分析,§17 的向上取整除法。

  \item \textbf{Wirth, N.} \emph{Algorithms + Data Structures = Programs}.
        Prentice-Hall, 1976.
        --- Bresenham 直线算法及其向圆周推广的经典论述。
\end{itemize}

\subsection*{补充在线资源}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item Block Round --- 应用程序: \biblink{https://vinisouza128.github.io/block-round/}{vinisouza128.github.io/block-round/}.
  \item Block Round --- 代码仓库: \biblink{https://github.com/ViniSouza128/block-round}{github.com/ViniSouza128/block-round}.
  \item 姊妹项目 Pixel Round(无 Minecraft 材质版本):
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.2em,topsep=0.3em]
  ...
  \item WorldEdit 文档, \biblink{https://enginehub.org/worldedit/}{enginehub.org/worldedit/} ── 本文档中使用的 \cmd{//sphere}、\cmd{//hsphere}、\cmd{//ellipsoid}、\cmd{//copy} 系列命令的官方参考。
  \item Sponge Schematic 规范 v2, \biblink{https://github.com/SpongePowered/Schematic-Specification}{github.com/SpongePowered/Schematic-Specification} ── Block Round 导出为 \code{.schem} 的文件格式。
\end{itemize}
...
Block Round 由 Vinícius Rodrigues de Souza 维护。姊妹项目 Pixel Round 可在 \biblink{https://github.com/ViniSouza128/pixel-round}{github.com/ViniSouza128/pixel-round} 获取；当前 Block Round 仓库位于 \biblink{https://github.com/ViniSouza128/block-round}{github.com/ViniSouza128/block-round}。
