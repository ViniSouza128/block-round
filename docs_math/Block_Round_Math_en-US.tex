...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{\emph{Thin (\emph{1 voxel})}}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.30\linewidth}\centering
  \includegraphics[width=\linewidth]{math_2d_d20_thick.png}
  \caption{\emph{Thick}}
\end{subfigure}
\caption{The three rendering modes for the same Euclidean circumference of $D=20$. The \emph{Thin} mode corresponds to the discrete topological boundary ($\partial F$); the \emph{Thick} mode adds diagonal cells to seal $45^\circ$ holes, essential for watertight builds (underwater Glass/Ice domes).}
\label{fig:modes}
\end{figure}
...
\begin{subfigure}[t]{0.46\linewidth}\centering
  ...
  \caption{Sphere $D=10$, $r_x=r_y=r_z=5$.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.46\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_ellipsoid.png}
  \caption{Ellipsoid $W=20,\,H=10,\,D=12$.}
\end{subfigure}
\caption{Voxelisation in $\mathbb{R}^3$. Each cube is a voxel; its centre coordinate $(i,j,k)$ is tested against the implicit equation $a_x^2+a_y^2+a_z^2\leq 1$. The visible surface staircases are a mathematical consequence of discretisation --- the very property that gives Minecraft its visual identity.}
\label{fig:3d-shapes}
\end{figure}
...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{Diagonal 50\%}
\end{subfigure}
\caption{Three cuts on the same $D=16$ sphere, each preserving 50\% of the volume. The $Y$ cut produces the classical dome $V_{\mathrm{dome}}=\tfrac{2}{3}\pi r^3$; the diagonal exposes the hypotenuse $x+y=k$ characteristic of that cutting plane.}
\label{fig:cuts}
\end{figure}
...
\]
The result is then intersected with the set preserved by the cut. The underlying theory is \acc{digital topology}: the thick operator guarantees \emph{26-connectivity} of the shell (neighbors including diagonals). This matters in Minecraft because even when two shell voxels are diagonal neighbours (sharing only an edge), the cell between them remains an air block that water and creatures cross into the interior --- thick mode closes those paths.

\begin{minec}
\textbf{Minecraft mapping.} For an underwater Glass dome on a survival ocean monument, thick mode is mandatory: the diagonal bridges fill the empty cells left by thin mode, eliminating the air-block paths through which water would otherwise invade. Cost: $D=20$ dome at thick mode requires $\approx 380$ Glass blocks vs.\ $\approx 320$ under thin mode --- $19\%$ more material in exchange for a completely closed shell.
\end{minec}
...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{\emph{Classic} (strong contrast)}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.30\linewidth}\centering
...
\end{subfigure}
\caption{Same voxel set with and without the black outline on exposed edges. With the \emph{highlight overlay} ON, a missing block in the shell yields a pattern discontinuity that the eye catches at a glance --- a quick validation tool during survival construction.}
\label{fig:overlay}
\end{figure}
...
\end{subfigure}
\caption{The same $D=8$ sphere rendered with three different blocks. The voxelised silhouette (mathematics) is identical across all three: the algorithm picks the voxels; the texture is a purely aesthetic layer applied afterwards that changes neither volume nor topology.}
\label{fig:textures}
\end{figure}
...
% =============================================================================
\section*{\color{accent}Bibliography}
\addcontentsline{toc}{section}{Bibliography}
\label{sec:bib}

\subsection*{Technical references (rasterisation and digital topology)}
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Bresenham, J. E.} \emph{Algorithm for computer control of a digital plotter}.
        ...
        Available at \biblink{https://doi.org/10.1147/sj.41.0025}{doi:10.1147/sj.41.0025}.
        --- original paper of the integer mid-point algorithm covered in §5.
  ...
  \item \textbf{Pitteway, M.~L.~V.} \emph{Algorithm for drawing ellipses or hyperbolae
        ...
        Available at \biblink{https://doi.org/10.1093/comjnl/10.3.282}{doi:10.1093/comjnl/10.3.282}.
        --- generalisation of the Bresenham algorithm to arbitrary conics, the basis of the ellipse extension described in §5.

  \item \textbf{Kappel, A.} \emph{An ellipse-drawing algorithm for raster displays}.
        In \emph{Fundamental Algorithms for Computer Graphics} (R.~A.~Earnshaw, ed.),
        NATO ASI Series F-17, Springer, 1985, p.~257--280.
        --- two-region formulation used in the ellipse implementation.

  \item \textbf{Klette, R.; Rosenfeld, A.} \emph{Digital Geometry: Geometric Methods
        for Digital Picture Analysis}. Morgan Kaufmann, 2004. 656 pages.
        --- standard reference for digital topology, boundary operators ($\partial F$), 4-/6-/26-connectivity. Used in §7, §12 and §18.

  \item \textbf{Foley, J.~D.; van Dam, A.; Feiner, S.~K.; Hughes, J.~F.}
        \emph{Computer Graphics: Principles and Practice}. 3rd ed., Addison-Wesley, 2014.
        --- classical treatment of rasterisation, lighting models, perspective projection. Relevant to §13--15.
\end{itemize}

\subsection*{Minecraft specifications and tools referenced}
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Sponge Project.} \emph{Sponge Schematic Specification, version~2}.
        ...
        --- file format \code{.schem} (gzip + NBT) that Block Round exports. Loadable by WorldEdit, Litematica and MCEdit.

  \item \textbf{EngineHub.} \emph{WorldEdit Documentation}.
        \biblink{https://worldedit.enginehub.org/}{worldedit.enginehub.org}.
        --- official reference for the \cmd{//sphere}, \cmd{//hsphere}, \cmd{//ellipsoid}, \cmd{//cyl}, \cmd{//schem load} commands cited throughout this document.

  \item \textbf{Mojang AB.} \emph{Minecraft Wiki --- Inventory}.
        \biblink{https://minecraft.wiki/w/Inventory}{minecraft.wiki/w/Inventory}.
        --- reference on slots, stacks, single chests (27 slots) and double chests (54 slots) used in §17.
  ...
  \item \textbf{Mojang AB.} \emph{Minecraft Wiki --- Commands/fill, /clone}.
        ...
        --- vanilla commands \cmd{/fill} and \cmd{/clone} cited in §17 and §20.

  \item \textbf{Masady} (\emph{Litematica} mod).
        \biblink{https://github.com/maruohon/litematica}{github.com/maruohon/litematica}.
        --- Minecraft Java Edition mod that imports schematics and displays a translucent ghost of the build to execute.
\end{itemize}

\subsection*{Geometry, linear algebra and group theory}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Strang, G.} \emph{Linear Algebra and Its Applications}. 4th ed.,
  ...
  \item \textbf{Armstrong, M.~A.} \emph{Groups and Symmetry}. Undergraduate Texts in
        ...
        --- introductory exposition of the octahedral group $O_h$ and its subgroups, used in §20.

  \item \textbf{Coxeter, H.~S.~M.} \emph{Regular Polytopes}. 3rd ed., Dover, 1973.
        --- canonical reference for the symmetry groups of regular solids, including the detailed treatment of $O_h$ of order 48 mentioned in §20.

  \item \textbf{Hilbert, D.; Cohn-Vossen, S.} \emph{Geometry and the Imagination}.
        AMS Chelsea, 1990 [orig.~1932].
        --- classical treatment of quadrics and geometric visualisation, intuitive basis for §9.
\end{itemize}

\subsection*{Algorithms, data structures, complexity}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Cormen, T.~H.; Leiserson, C.~E.; Rivest, R.~L.; Stein, C.}
        \emph{Introduction to Algorithms}. 4th ed., MIT Press, 2022.
        --- complexity analysis of the three algorithms in §4--6, ceiling division in §17.

  \item \textbf{Wirth, N.} \emph{Algorithms + Data Structures = Programs}.
        Prentice-Hall, 1976.
        --- classical exposition of the Bresenham algorithm for lines and its extension to circumferences.
\end{itemize}

\subsection*{Complementary online resources}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item Block Round --- app: \biblink{https://vinisouza128.github.io/block-round/}{vinisouza128.github.io/block-round/}.
  \item Block Round --- repository: \biblink{https://github.com/ViniSouza128/block-round}{github.com/ViniSouza128/block-round}.
  \item Sibling project Pixel Round (Minecraft-texture-free variant):
        \biblink{https://github.com/ViniSouza128/pixel-round}{github.com/ViniSouza128/pixel-round}.
  \item Lesson plan in pt-BR (3rd year of secondary school):
...
The table below extends the canonical sphere tables of Sections~10 and~17 by adding the hollow-shell variant (\emph{thin} mode) alongside the filled volume. The hollow-shell counts assume a single-voxel shell and are computed under the Euclidean algorithm; the thick mode adds roughly $10{-}15\%$ more blocks to fill the empty diagonal cells as described in Section~12.
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.2em,topsep=0.3em]
  ...
  \item WorldEdit documentation, \biblink{https://enginehub.org/worldedit/}{enginehub.org/worldedit/} --- official command reference for the \cmd{//sphere}, \cmd{//hsphere}, \cmd{//ellipsoid}, \cmd{//copy} family used throughout this document.
  \item Sponge Schematic specification v2, \biblink{https://github.com/SpongePowered/Schematic-Specification}{github.com/SpongePowered/Schematic-Specification} --- the file format Block Round exports as \code{.schem}.
\end{itemize}
...
Block Round is maintained by Vinícius Rodrigues de Souza. The companion project Pixel Round is available at \biblink{https://github.com/ViniSouza128/pixel-round}{github.com/ViniSouza128/pixel-round}; the current Block Round repository is at \biblink{https://github.com/ViniSouza128/block-round}{github.com/ViniSouza128/block-round}.
