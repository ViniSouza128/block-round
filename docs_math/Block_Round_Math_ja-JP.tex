...
再現性に関する最終的な注釈：本文書のパイプライン全体 ── ビルドスクリプト、翻訳、LaTeX テンプレート、ロケール別表 ── はプロジェクトリポジトリの \code{docs_math/} ディレクトリに含まれる。XeLaTeX/MiKTeX を備えたシステム上で \code{python build.py} を再実行すると、サポートされている 9 つのロケールのいずれでも、この PDF を正確に生成する。姉妹プロジェクト Pixel Round は同じ構造に従い、テクスチャ付きとテクスチャなしバリアントの直接の側並びの比較を可能にする。
