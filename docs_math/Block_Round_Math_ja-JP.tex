...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{\emph{しきい値}}
\end{subfigure}
\caption{3 つの基準による直径 $D=10$ の同一円周。各図は対応する不等式 $d_x^2+d_y^2\leq 1$ により選ばれたセル集合を示し,Minecraft の \emph{cobblestone} テクスチャで描画されている。行間の遷移,角セル,基本方位の違いに注目——いずれも異なる基準のもとで数学的に正しい 3 つの答えである。}
\label{fig:comp-d10}
\end{figure}
...
\begin{minec}
\textbf{Minecraft へのマッピング。} $D=20$ のガラスドーム（中空）について、太モードは $608$ ブロックの \emph{気密な} 外殻を生むのに対し、細モードでは約 $510$ ブロック ─── 余分な $\sim 100$ ブロックは対角橋であり,細モードが残す対角に空いたセル(空気ブロック)を通って環境光が漏れるのを防ぐ。
\end{minec}
...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{\emph{細 (\emph{1 ボクセル})}}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.30\linewidth}\centering
  \includegraphics[width=\linewidth]{math_2d_d20_thick.png}
  \caption{\emph{太}}
\end{subfigure}
\caption{同一の $D=20$ ユークリッド円周に対する 3 つの描画モード。\emph{細}モードは離散位相境界 ($\partial F$) に対応し,\emph{太}モードは $45^\circ$ の穴を塞ぐ対角ブロックを追加する——水中のガラス/氷ドームなど密閉構造には必須である。}
\label{fig:modes}
\end{figure}
...
\begin{subfigure}[t]{0.46\linewidth}\centering
  ...
  \caption{球 $D=10$, $r_x=r_y=r_z=5$.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.46\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_ellipsoid.png}
  \caption{楕円体 $W=20,\,H=10,\,D=12$.}
\end{subfigure}
\caption{$\mathbb{R}^3$ におけるボクセル化。各立方体が 1 ボクセルで,その中心座標 $(i,j,k)$ を陰関数 $a_x^2+a_y^2+a_z^2\leq 1$ に代入して判定する。表面に見える階段状は離散化の数学的帰結である——まさに Minecraft に視覚的アイデンティティを与えている特性に他ならない。}
\label{fig:3d-shapes}
\end{figure}
...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{対角 50\%}
\end{subfigure}
\caption{同一の $D=16$ 球に対する 3 通りの切断で,いずれも体積の 50\% を保持する。$Y$ 切断は古典的なドーム $V_{\mathrm{dome}}=\tfrac{2}{3}\pi r^3$ を与え,対角切断はその切断面に特徴的な斜辺 $x+y=k$ を露出させる。}
\label{fig:cuts}
\end{figure}
...
\]
結果は切断によって保存された集合と共通部分を取る。背景理論は \acc{数字位相}：太演算子はシェルの \emph{26-連結性}（対角を含む隣接）を保証する。これは Minecraft 建築で有用である。なぜなら対角に隣接するボクセルは面で触れない ─── 光,水,生物がこれらの対角に空いたセル(空気ブロック)を壁がないかのように通過する。
...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{\emph{Classic} (強コントラスト)}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.30\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_shading_blocks.png}
  \caption{\emph{Blocks} (デフォルト)}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.30\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_shading_smooth.png}
  \caption{\emph{Smooth} (面が近似)}
\end{subfigure}
\caption{同じ 3 つの可視面に対する 3 種類の光乗数。\emph{Classic} は Minecraft バニラの定番外観,\emph{Smooth} は滑らかな表面のためにコントラストを下げ,\emph{Blocks} は同一テクスチャでもボクセル境界を浮き上がらせる幾何的インセットを伴う中間デフォルトである。}
\label{fig:shading}
\end{figure}
...
\begin{subfigure}[t]{0.40\linewidth}\centering
  ...
  \caption{オーバーレイ \emph{OFF}}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.40\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_overlay_on.png}
  \caption{オーバーレイ \emph{ON}(デフォルト)}
\end{subfigure}
\caption{露出辺の黒い輪郭の有無による同一ボクセル集合の対比。\emph{highlight overlay} を ON にすると,殻に欠けたブロックがあると一目でわかるパターンの不連続が生じる——サバイバル建築中の素早い検証手段である。}
\label{fig:overlay}
\end{figure}
...
\caption{$D=10$ の球で,正の八分象限 $(+x,+y,+z)$ を緑で強調表示。部分群 $\mathbb{Z}_2^3 \leq O_h$(位数 8,3 つの座標反射で生成)の作用のもと,この八分象限が球全体を決定する——他の 7 領域は手作業で構築した八分象限に対し \emph{mode:masked} 付きの \cmd{/clone} を順に適用して得られる。}
\label{fig:octants}
\end{figure}
...
\end{subfigure}
\caption{同一の $D=8$ 球を 3 種類の異なるブロックで描画。ボクセル化されたシルエット(数学)は 3 つとも同一であり,アルゴリズムがボクセルを選び,テクスチャは体積も位相も変えない後付けの純粋に美的なレイヤーに過ぎない。}
\label{fig:textures}
\end{figure}
...
% =============================================================================
\section*{\color{accent}参考文献}
\addcontentsline{toc}{section}{参考文献}
\label{sec:bib}

\subsection*{技術文献(ラスタライズと離散位相)}
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Bresenham, J. E.} \emph{Algorithm for computer control of a digital plotter}.
        ...
        入手先 \biblink{https://doi.org/10.1147/sj.41.0025}{doi:10.1147/sj.41.0025}.
        --- 本書 §5 で扱う整数ミッドポイント・アルゴリズムの原論文。
  ...
  \item \textbf{Pitteway, M.~L.~V.} \emph{Algorithm for drawing ellipses or hyperbolae
        ...
        入手先 \biblink{https://doi.org/10.1093/comjnl/10.3.282}{doi:10.1093/comjnl/10.3.282}.
        --- Bresenham アルゴリズムの任意の円錐曲線への一般化で,§5 で述べる楕円拡張の基礎。

  \item \textbf{Kappel, A.} \emph{An ellipse-drawing algorithm for raster displays}.
        所収 \emph{Fundamental Algorithms for Computer Graphics} (R.~A.~Earnshaw, ed.),
        NATO ASI Series F-17, Springer, 1985, p.~257--280.
        --- 楕円実装で用いる二領域定式化。

  \item \textbf{Klette, R.; Rosenfeld, A.} \emph{Digital Geometry: Geometric Methods
        for Digital Picture Analysis}. Morgan Kaufmann, 2004. 656 ページ.
        --- 離散位相,境界作用素 ($\partial F$),4-/6-/26-連結性の標準参考。§7,§12,§18 で使用。

  \item \textbf{Foley, J.~D.; van Dam, A.; Feiner, S.~K.; Hughes, J.~F.}
        \emph{Computer Graphics: Principles and Practice}. 第3版, Addison-Wesley, 2014.
        --- ラスタライズ,照明モデル,透視投影の古典的論述。§13--15 に関連。
\end{itemize}

\subsection*{参照した Minecraft の仕様とツール}
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  ...
  \item \textbf{Mojang AB.} \emph{Minecraft Wiki --- Commands/fill, /clone}.
        ...
        --- §17 および §20 で引用するバニラコマンド \cmd{/fill} と \cmd{/clone}。

  \item \textbf{Masady} (\emph{Litematica} mod).
        \biblink{https://github.com/maruohon/litematica}{github.com/maruohon/litematica}.
        --- スキーマティックを読み込み,施工対象の半透明なゴーストを表示する Minecraft Java Edition の mod。
\end{itemize}

\subsection*{幾何学,線形代数,群論}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{齋藤正彦} \emph{線型代数入門}. 東京大学出版会, 1966.
  ...
  \item \textbf{Armstrong, M.~A.} \emph{Groups and Symmetry}. Undergraduate Texts in
        ...
        --- 八面体群 $O_h$ とその部分群の入門的解説,§20 で使用。

  \item \textbf{Coxeter, H.~S.~M.} \emph{Regular Polytopes}. 第3版, Dover, 1973.
        --- 正多面体の対称群の標準的参考。§20 で言及される位数 48 の $O_h$ の詳細な扱いを含む。

  \item \textbf{Hilbert, D.; Cohn-Vossen, S.} \emph{Geometry and the Imagination}.
        AMS Chelsea, 1990 [orig.~1932].
        --- 二次曲面と幾何学的可視化の古典的論述,§9 の直観的基礎。
\end{itemize}

\subsection*{アルゴリズム,データ構造,計算量}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Cormen, T.~H.; Leiserson, C.~E.; Rivest, R.~L.; Stein, C.}
        \emph{Introduction to Algorithms}. 第4版, MIT Press, 2022.
        --- §4--6 の 3 アルゴリズムの計算量解析と,§17 の天井除算。

  \item \textbf{Wirth, N.} \emph{Algorithms + Data Structures = Programs}.
        Prentice-Hall, 1976.
        --- Bresenham の直線アルゴリズムとその円周への拡張についての古典的論述。
\end{itemize}

\subsection*{補足オンライン資料}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item Block Round --- アプリ: \biblink{https://vinisouza128.github.io/block-round/}{vinisouza128.github.io/block-round/}.
  \item Block Round --- リポジトリ: \biblink{https://github.com/ViniSouza128/block-round}{github.com/ViniSouza128/block-round}.
  \item 姉妹プロジェクト Pixel Round(Minecraft テクスチャ非使用版):
        \biblink{https://github.com/ViniSouza128/pixel-round}{github.com/ViniSouza128/pixel-round}.
  \item pt-BR の指導案(高校3年生):
...
以下の表は第 10 節と第 17 節の標準球表を拡張し、中空殻バリアント（\emph{細}モード）を充填体積と並べて記載する。中空殻のカウントは 1 ボクセル厚を仮定しユークリッドアルゴリズムで計算される；太モードは第 12 節の説明通り対角に空いたセル(空気ブロック)を埋めるため約 $10{-}15\%$ 多くのブロックを加える。
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.2em,topsep=0.3em]
  ...
  \item WorldEdit ドキュメント, \biblink{https://enginehub.org/worldedit/}{enginehub.org/worldedit/} ── 本文書全体で使用される \cmd{//sphere}、\cmd{//hsphere}、\cmd{//ellipsoid}、\cmd{//copy} 系コマンドの公式参照。
  \item Sponge Schematic 仕様 v2, \biblink{https://github.com/SpongePowered/Schematic-Specification}{github.com/SpongePowered/Schematic-Specification} ── Block Round が \code{.schem} としてエクスポートするファイル形式。
\end{itemize}
...
Block Round は Vinícius Rodrigues de Souza によって維持されている。姉妹プロジェクト Pixel Round は \biblink{https://github.com/ViniSouza128/pixel-round}{github.com/ViniSouza128/pixel-round} で利用可能；現在の Block Round リポジトリは \biblink{https://github.com/ViniSouza128/block-round}{github.com/ViniSouza128/block-round} にある。
