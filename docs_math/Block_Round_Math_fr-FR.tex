...
\begin{subfigure}[t]{0.46\linewidth}\centering
  ...
  \caption{Sphère $D=10$, $r_x=r_y=r_z=5$.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.46\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_ellipsoid.png}
  \caption{Ellipsoïde $W=20,\,H=10,\,D=12$.}
\end{subfigure}
\caption{Voxélisation dans $\mathbb{R}^3$. Chaque cube est un voxel ; sa coordonnée centrale $(i,j,k)$ est testée dans l'équation implicite $a_x^2+a_y^2+a_z^2\leq 1$. Les escaliers visibles à la surface sont une conséquence mathématique de la discrétisation --- précisément la propriété qui donne à Minecraft son identité visuelle.}
\label{fig:3d-shapes}
\end{figure}
...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{Diagonale 50\%}
\end{subfigure}
\caption{Trois coupes sur la même sphère $D=16$, chacune préservant 50\% du volume. La coupe $Y$ produit le dôme classique $V_{\mathrm{dôme}}=\tfrac{2}{3}\pi r^3$ ; la diagonale expose l'hypoténuse $x+y=k$ caractéristique de ce plan de coupe.}
\label{fig:cuts}
\end{figure}
...
\begin{minec}
\textbf{Mapping Minecraft.} Pour un dôme de Verre sous-marin sur un monument océanique en survie, le mode épais est obligatoire : les ponts diagonaux empêchent l'eau d'entrer par les cellules vides que le mode mince laisse à $45^\circ$. Coût : dôme $D=20$ en épais demande $\approx 380$ blocs de Verre contre $\approx 320$ en mince --- $19\%$ de matière en plus pour l'étanchéité totale.
\end{minec}
...
\begin{subfigure}[t]{0.40\linewidth}\centering
  ...
  \caption{Surimpression \emph{OFF}}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.40\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_overlay_on.png}
  \caption{Surimpression \emph{ON} (par défaut)}
\end{subfigure}
\caption{Même ensemble de voxels avec et sans le contour noir sur les arêtes exposées. Avec le \emph{highlight overlay} ON, un bloc manquant dans la coque produit une discontinuité dans le motif que l'œil détecte d'un coup d'œil --- un outil de validation rapide lors de la construction en survie.}
\label{fig:overlay}
\end{figure}
...
\end{subfigure}
\caption{La même sphère de diamètre $D=8$ rendue avec trois blocs différents. La silhouette voxélisée (la mathématique) est identique dans les trois : l'algorithme choisit les voxels ; la texture est une couche purement esthétique appliquée ensuite qui ne modifie ni le volume ni la topologie.}
\label{fig:textures}
\end{figure}
...
% =============================================================================
\section*{\color{accent}Bibliographie}
\addcontentsline{toc}{section}{Bibliographie}
\label{sec:bib}

\subsection*{Références techniques (rastérisation et topologie numérique)}
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Bresenham, J. E.} \emph{Algorithm for computer control of a digital plotter}.
        ...
        Disponible sur \biblink{https://doi.org/10.1147/sj.41.0025}{doi:10.1147/sj.41.0025}.
        --- article original de l'algorithme du point médian entier traité au §5.
  ...
  \item \textbf{Pitteway, M.~L.~V.} \emph{Algorithm for drawing ellipses or hyperbolae
        ...
        Disponible sur \biblink{https://doi.org/10.1093/comjnl/10.3.282}{doi:10.1093/comjnl/10.3.282}.
        --- généralisation de l'algorithme de Bresenham aux coniques arbitraires, base de l'extension aux ellipses décrite au §5.

  \item \textbf{Kappel, A.} \emph{An ellipse-drawing algorithm for raster displays}.
        Dans \emph{Fundamental Algorithms for Computer Graphics} (R.~A.~Earnshaw, ed.),
        NATO ASI Series F-17, Springer, 1985, p.~257--280.
        --- formulation à deux régions utilisée dans l'implémentation des ellipses.
  ...
  \item \textbf{Klette, R.; Rosenfeld, A.} \emph{Digital Geometry: Geometric Methods
        ...
        --- référence standard pour la topologie numérique, opérateurs de bord ($\partial F$), 4-/6-/26-connexité. Utilisée aux §7, §12 et §18.

  \item \textbf{Foley, J.~D.; van Dam, A.; Feiner, S.~K.; Hughes, J.~F.}
        \emph{Computer Graphics: Principles and Practice}. 3\textsuperscript{e} éd., Addison-Wesley, 2014.
        --- traitement classique de la rastérisation, des modèles d'éclairage, de la projection en perspective. Pertinent pour les §13--15.
\end{itemize}

\subsection*{Spécifications et outils Minecraft référencés}
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Sponge Project.} \emph{Sponge Schematic Specification, version~2}.
        ...
        --- format de fichier \code{.schem} (gzip + NBT) exporté par Block Round. Chargeable par WorldEdit, Litematica et MCEdit.

  \item \textbf{EngineHub.} \emph{WorldEdit Documentation}.
        \biblink{https://worldedit.enginehub.org/}{worldedit.enginehub.org}.
        --- référence officielle pour les commandes \cmd{//sphere}, \cmd{//hsphere}, \cmd{//ellipsoid}, \cmd{//cyl}, \cmd{//schem load} citées tout au long du document.

  \item \textbf{Mojang AB.} \emph{Minecraft Wiki --- Inventory}.
        \biblink{https://minecraft.wiki/w/Inventory}{minecraft.wiki/w/Inventory}.
        --- référence sur les emplacements, piles, coffres simples (27 emplacements) et coffres doubles (54 emplacements) utilisés au §17.
  ...
  \item \textbf{Mojang AB.} \emph{Minecraft Wiki --- Commands/fill, /clone}.
        ...
        --- commandes vanilla \cmd{/fill} et \cmd{/clone} citées aux §17 et §20.

  \item \textbf{Masady} (\emph{Litematica} mod).
        \biblink{https://github.com/maruohon/litematica}{github.com/maruohon/litematica}.
        --- mod de Minecraft Java Edition qui importe des schematics et affiche un fantôme translucide de la construction à exécuter.
\end{itemize}

\subsection*{Géométrie, algèbre linéaire et théorie des groupes}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Audin, M.} \emph{Géométrie}. EDP Sciences, 2006.
  ...
  \item \textbf{Armstrong, M.~A.} \emph{Groups and Symmetry}. Undergraduate Texts in
        ...
        --- exposition introductive au groupe octaédrique $O_h$ et à ses sous-groupes, utilisée au §20.

  \item \textbf{Coxeter, H.~S.~M.} \emph{Regular Polytopes}. 3\textsuperscript{e} éd., Dover, 1973.
        --- référence canonique pour les groupes de symétrie des solides réguliers, incluant le traitement détaillé de $O_h$ d'ordre 48 mentionné au §20.

  \item \textbf{Hilbert, D.; Cohn-Vossen, S.} \emph{Geometry and the Imagination}.
        AMS Chelsea, 1990 [orig.~1932].
        --- traitement classique des quadriques et de la visualisation géométrique, base intuitive du §9.
\end{itemize}

\subsection*{Algorithmes, structures de données, complexité}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Cormen, T.~H.; Leiserson, C.~E.; Rivest, R.~L.; Stein, C.}
        \emph{Introduction to Algorithms}. 4\textsuperscript{e} éd., MIT Press, 2022.
        --- analyse de complexité des trois algorithmes des §4--6, division avec plafond du §17.

  \item \textbf{Wirth, N.} \emph{Algorithms + Data Structures = Programs}.
        Prentice-Hall, 1976.
        --- exposition classique de l'algorithme de Bresenham pour les droites et son extension aux circonférences.
\end{itemize}

\subsection*{Ressources en ligne complémentaires}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item Block Round --- application: \biblink{https://vinisouza128.github.io/block-round/}{vinisouza128.github.io/block-round/}.
  \item Block Round --- dépôt: \biblink{https://github.com/ViniSouza128/block-round}{github.com/ViniSouza128/block-round}.
  \item Projet frère Pixel Round (variante sans textures Minecraft):
        \biblink{https://github.com/ViniSouza128/pixel-round}{github.com/ViniSouza128/pixel-round}.
  \item Plan de cours en pt-BR (3\textsuperscript{e} année de lycée) :
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.2em,topsep=0.3em]
  ...
  \item Documentation WorldEdit, \biblink{https://enginehub.org/worldedit/}{enginehub.org/worldedit/} --- référence officielle des commandes \cmd{//sphere}, \cmd{//hsphere}, \cmd{//ellipsoid}, \cmd{//copy} utilisées tout au long de ce document.
  \item Spécification Sponge Schematic v2, \biblink{https://github.com/SpongePowered/Schematic-Specification}{github.com/SpongePowered/Schematic-Specification} --- le format de fichier que Block Round exporte comme \code{.schem}.
\end{itemize}
...
Block Round est maintenu par Vinícius Rodrigues de Souza. Le projet frère Pixel Round est disponible à \biblink{https://github.com/ViniSouza128/pixel-round}{github.com/ViniSouza128/pixel-round} ; le dépôt actuel de Block Round est à \biblink{https://github.com/ViniSouza128/block-round}{github.com/ViniSouza128/block-round}.
