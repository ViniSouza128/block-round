...
\begin{subfigure}[t]{0.46\linewidth}\centering
  ...
  \caption{Kugel $D=10$, $r_x=r_y=r_z=5$.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.46\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_ellipsoid.png}
  \caption{Ellipsoid $W=20,\,H=10,\,D=12$.}
\end{subfigure}
\caption{Voxelisierung im $\mathbb{R}^3$. Jeder Würfel ist ein Voxel; seine Mittelpunktskoordinate $(i,j,k)$ wird gegen die implizite Gleichung $a_x^2+a_y^2+a_z^2\leq 1$ geprüft. Die sichtbaren Treppenstufen auf der Oberfläche sind eine mathematische Folge der Diskretisierung --- genau die Eigenschaft, die Minecraft seine visuelle Identität verleiht.}
\label{fig:3d-shapes}
\end{figure}
...
\begin{subfigure}[t]{0.30\linewidth}\centering
  ...
  \caption{$Y$-Schnitt 50\% (Kuppel)}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.30\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_cut_x.png}
  \caption{$X$-Schnitt 50\%}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.30\linewidth}\centering
  \includegraphics[width=\linewidth]{math_3d_cut_diag.png}
  \caption{Diagonal 50\%}
\end{subfigure}
\caption{Drei Schnitte durch dieselbe Kugel $D=16$, jeder erhält 50\% des Volumens. Der $Y$-Schnitt erzeugt die klassische Kuppel $V_{\mathrm{Kuppel}}=\tfrac{2}{3}\pi r^3$; der diagonale Schnitt legt die Hypotenuse $x+y=k$ frei, die für diese Schnittebene charakteristisch ist.}
\label{fig:cuts}
\end{figure}
...
\begin{subfigure}[t]{0.40\linewidth}\centering
  ...
  \caption{Overlay \emph{ON} (Standard)}
\end{subfigure}
\caption{Dieselbe Voxelmenge mit und ohne schwarze Umrandung an den freiliegenden Kanten. Bei aktiviertem \emph{Highlight-Overlay} erzeugt ein fehlender Block in der Schale eine Diskontinuität im Muster, die das Auge sofort erkennt --- ein schnelles Validierungswerkzeug während des Bauens im Survival-Modus.}
\label{fig:overlay}
\end{figure}
...
% =============================================================================
\section*{\color{accent}Literaturverzeichnis}
\addcontentsline{toc}{section}{Literaturverzeichnis}
\label{sec:bib}

\subsection*{Technische Referenzen (Rasterisierung und digitale Topologie)}
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Bresenham, J. E.} \emph{Algorithm for computer control of a digital plotter}.
        ...
        Verfügbar unter \biblink{https://doi.org/10.1147/sj.41.0025}{doi:10.1147/sj.41.0025}.
        --- Originalartikel des ganzzahligen Mittelpunkt-Algorithmus, behandelt in §5.
  ...
  \item \textbf{Pitteway, M.~L.~V.} \emph{Algorithm for drawing ellipses or hyperbolae
        ...
        Verfügbar unter \biblink{https://doi.org/10.1093/comjnl/10.3.282}{doi:10.1093/comjnl/10.3.282}.
        --- Verallgemeinerung des Bresenham-Algorithmus auf beliebige Kegelschnitte, Grundlage der in §5 beschriebenen Ellipsenerweiterung.
  ...
  \item \textbf{Kappel, A.} \emph{An ellipse-drawing algorithm for raster displays}.
        ...
        --- Zwei-Regionen-Formulierung, die in der Ellipsenimplementierung verwendet wird.

  \item \textbf{Klette, R.; Rosenfeld, A.} \emph{Digital Geometry: Geometric Methods
        for Digital Picture Analysis}. Morgan Kaufmann, 2004. 656 Seiten.
        --- Standardreferenz für digitale Topologie, Randoperatoren ($\partial F$), 4-/6-/26-Nachbarschaft. Verwendet in §7, §12 und §18.

  \item \textbf{Foley, J.~D.; van Dam, A.; Feiner, S.~K.; Hughes, J.~F.}
        \emph{Computer Graphics: Principles and Practice}. 3. Aufl., Addison-Wesley, 2014.
        --- Klassische Behandlung von Rasterisierung, Beleuchtungsmodellen und perspektivischer Projektion. Relevant für §13--15.
\end{itemize}

\subsection*{Referenzierte Minecraft-Spezifikationen und Werkzeuge}
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Sponge Project.} \emph{Sponge Schematic Specification, version~2}.
        ...
        --- Dateiformat \code{.schem} (gzip + NBT), das Block Round exportiert. Ladbar mit WorldEdit, Litematica und MCEdit.

  \item \textbf{EngineHub.} \emph{WorldEdit Documentation}.
        \biblink{https://worldedit.enginehub.org/}{worldedit.enginehub.org}.
        --- Offizielle Referenz für die Befehle \cmd{//sphere}, \cmd{//hsphere}, \cmd{//ellipsoid}, \cmd{//cyl}, \cmd{//schem load}, die in diesem Dokument zitiert werden.

  \item \textbf{Mojang AB.} \emph{Minecraft Wiki --- Inventory}.
        \biblink{https://minecraft.wiki/w/Inventory}{minecraft.wiki/w/Inventory}.
        --- Referenz zu Slots, Stacks, einfachen Truhen (27 Slots) und Doppeltruhen (54 Slots), verwendet in §17.
  ...
  \item \textbf{Mojang AB.} \emph{Minecraft Wiki --- Commands/fill, /clone}.
        ...
        --- Vanilla-Befehle \cmd{/fill} und \cmd{/clone}, zitiert in §17 und §20.

  \item \textbf{Masady} (\emph{Litematica} mod).
        \biblink{https://github.com/maruohon/litematica}{github.com/maruohon/litematica}.
        --- Minecraft-Java-Edition-Mod, der Schematics importiert und einen durchscheinenden Geist des auszuführenden Bauwerks anzeigt.
\end{itemize}

\subsection*{Geometrie, lineare Algebra und Gruppentheorie}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Fischer, G.} \emph{Analytische Geometrie}. Vieweg, 1985.
  ...
  \item \textbf{Armstrong, M.~A.} \emph{Groups and Symmetry}. Undergraduate Texts in
        ...
        --- einführende Darstellung der oktaedrischen Gruppe $O_h$ und ihrer Untergruppen, verwendet in §20.

  \item \textbf{Coxeter, H.~S.~M.} \emph{Regular Polytopes}. 3. Aufl., Dover, 1973.
        --- kanonische Referenz für die Symmetriegruppen regulärer Körper, einschließlich der detaillierten Behandlung von $O_h$ der Ordnung 48, die in §20 erwähnt wird.

  \item \textbf{Hilbert, D.; Cohn-Vossen, S.} \emph{Geometry and the Imagination}.
        AMS Chelsea, 1990 [orig.~1932].
        --- klassische Behandlung von Quadriken und geometrischer Visualisierung, intuitive Grundlage für §9.
\end{itemize}

\subsection*{Algorithmen, Datenstrukturen, Komplexität}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item \textbf{Cormen, T.~H.; Leiserson, C.~E.; Rivest, R.~L.; Stein, C.}
        \emph{Introduction to Algorithms}. 4. Aufl., MIT Press, 2022.
        --- Komplexitätsanalyse der drei Algorithmen aus §4--6, Aufrundungsdivision aus §17.

  \item \textbf{Wirth, N.} \emph{Algorithms + Data Structures = Programs}.
        Prentice-Hall, 1976.
        --- klassische Darstellung des Bresenham-Algorithmus für Geraden und seiner Erweiterung auf Kreise.
\end{itemize}

\subsection*{Ergänzende Online-Ressourcen}

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em,topsep=0.3em]
  \item Block Round --- Anwendung: \biblink{https://vinisouza128.github.io/block-round/}{vinisouza128.github.io/block-round/}.
  \item Block Round --- Repository: \biblink{https://github.com/ViniSouza128/block-round}{github.com/ViniSouza128/block-round}.
  \item Schwesterprojekt Pixel Round (Variante ohne Minecraft-Texturen):
        \biblink{https://github.com/ViniSouza128/pixel-round}{github.com/ViniSouza128/pixel-round}.
  \item Unterrichtsplan auf pt-BR (3. Jahr der Sekundarstufe):
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.2em,topsep=0.3em]
  ...
  \item WorldEdit-Dokumentation, \biblink{https://enginehub.org/worldedit/}{enginehub.org/worldedit/} --- offizielle Befehlsreferenz für die in diesem Dokument verwendeten Befehle \cmd{//sphere}, \cmd{//hsphere}, \cmd{//ellipsoid}, \cmd{//copy}.
  \item Sponge Schematic Specification v2, \biblink{https://github.com/SpongePowered/Schematic-Specification}{github.com/SpongePowered/Schematic-Specification} --- das Dateiformat, das Block Round als \code{.schem} exportiert.
\end{itemize}
...
Block Round wird von Vinícius Rodrigues de Souza gepflegt. Das Schwesterprojekt Pixel Round ist unter \biblink{https://github.com/ViniSouza128/pixel-round}{github.com/ViniSouza128/pixel-round} verfügbar; das aktuelle Block-Round-Repository ist unter \biblink{https://github.com/ViniSouza128/block-round}{github.com/ViniSouza128/block-round}.
